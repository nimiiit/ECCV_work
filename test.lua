...
---load the generator and reblur module (for testing)
opt.netG_name = opt.name .. '/' .. opt.which_epoch .. '_net_G'
opt.netRevG_name = opt.name .. '/' .. opt.which_epoch .. '_net_RevG'
...
netG:evaluate()
local netRevG = util.load(paths.concat(opt.checkpoints_dir, opt.netRevG_name .. '.t7'), opt)
netRevG:evaluate()
...
for n=1,math.floor(opt.how_many/opt.batchSize) do
    ...
    input = data_curr[{ {}, idx_A, {}, {} }] --blurred input
    target = data_curr[{ {}, idx_B, {}, {} }]  ---target clean image
    ...
    if opt.preprocess == 'colorization' then
       ...
       input = util.deprocessL_batch(input_L)
        ...
        output = util.deprocess_batch(netG:forward(input))    -- util.deprocess_batch(netRevG:forward(netG:forward(input)))  --
